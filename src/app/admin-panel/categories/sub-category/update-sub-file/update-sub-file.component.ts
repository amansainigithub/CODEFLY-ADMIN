...
@Component({
  selector: 'app-update-sub-file',
  templateUrl: './update-sub-file.component.html',
  styleUrl: './update-sub-file.component.css'
})
export class UpdateSubFileComponent {
    ...
    updateCategoryFile(){
      if(this.file == null)
      {
       this.toast.error({detail:"Error",summary:"Error : File is Empty", position:"bottomRight",duration:3000});
        ...
        this.subCategoryService.updateSubFile(this.file,this.data.subCategoryId).subscribe({
          next:(res:any)=>{
            console.log(res);
