...
export class ProductNetQuantityComponent {
  ...
  getNetQuantityPagination(request: any) {
    this.spinner.show();
    this.netQuantityService.getNetQuantityByPagination(request).subscribe({
      next: (res: any) => {
  ...
  saveNetQuantity() {
    ...
    this.netQuantityService.saveNetQuantity(this.form).subscribe({
      next: (res: any) => {
        this.toast.success({
          detail: 'Success',
          summary: 'Net Quantity Saved Success',
          position: 'bottomRight',
          duration: 3000,
        });
        this.spinner.hide();
  ...
  deleteNetQuantityByid(netQuantityId: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        //Delete net quantity
        this.netQuantityService
          .deleteNetQuantityByIdService(netQuantityId)
          .subscribe({
            next: (res: any) => {
              this.toast.success({
                detail: 'Success',
